...
const LOG_PREFIX = "upgraded-tray",
  LOG_LOADING = "loading",
...
const addQuickNote = () => {
    // Создаём быструю заметку через команду QuickAdd «Создать новую заметку».
    // Вызывается как обычная команда Obsidian по её ID, поэтому работает и
    // когда окно свёрнуто в трей: сначала показываем и фокусируем окно, затем
    // (после небольшой задержки на отрисовку) запускаем команду.
    const commandId =
      plugin.settings.quickNoteCommandId ||
      "quickadd:choice:dad1bbda-6f81-4286-8102-d99c69d32624";
    showWindows();
...
const OPTIONS = [
  ...
  "Quick notes",
  {
    ...
    desc: `
      ID команды Obsidian, которая создаёт быструю заметку. По умолчанию —
      QuickAdd «Создать новую заметку». Узнать ID другой команды можно в
      <code>.obsidian/hotkeys.json</code> или в консоли разработчика через
      <code>app.commands.listCommands()</code>.
    `,
    type: "text",
    default: "quickadd:choice:dad1bbda-6f81-4286-8102-d99c69d32624",
  },
  {
    key: "quickNoteHotkey",
    desc: ACCELERATOR_FORMAT,
